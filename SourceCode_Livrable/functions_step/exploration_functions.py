...
def common_points_plot(frequency_df):
    ...
    for column in frequency_df:
        frequency_df[column].to_csv(f'fig/Mesures_frequences_valeurs/{column}_frequency.txt')
    ...
    for column in frequency_df:
        ...
        plt.tight_layout()
        plt.savefig(f'fig/Plots_frequences_valeurs/{column}_frequency_plot.png')
        plt.close()
